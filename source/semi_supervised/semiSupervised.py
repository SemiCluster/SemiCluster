...
'''
    A function to calculate the distance between two data points of a matrix, applicable to density peak distance discovery data structures
...
def SSC_DensityPeaks_SVC(train, label_train, t, nneigh, clf): # , test, label_test #, clf, n, decay_choice, contribute_error_rate
    ...
    while (len(struct) > 0):
        ...
        for i in range(0, len(struct), 1):
            ...
            # print("number_neigh_test", number_neigh_test)
            number_neigh = find(nneigh == struct[i])[0]
            # print("number_neigh", number_neigh)
        ...
        # print("data_neigh", data_neigh.shape)
        struct = np.setdiff1d(data_neigh, struct_record)
        length_struct = len(struct)
...
def SSC_DensityPeaks_SVC_ensemble(train_1, label_train_1, train_2, label_train_2, t, nneigh_x, nneigh_z, clf1, clf2):
    ...
    '''Randomisation of data and labels'''
    train_x = train_1
