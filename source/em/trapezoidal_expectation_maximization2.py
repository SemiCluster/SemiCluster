from source.transforms.trapezoidal_transform_function2 import TrapezoidalTransformFunction2
import numpy as np
...
class TrapezoidalExpectationMaximization2(ExpectationMaximization):
    def __init__(self, cont_indices, ord_indices, window_size, window_width, sigma_init=None):
        self.transform_function = TrapezoidalTransformFunction2(cont_indices, ord_indices, window_size = window_size, window_width = window_width)
    ...
    def partial_fit_and_predict(self, X_batch, cont_indices,  ord_indices, max_workers=4, num_ord_updates=2,
                                decay_coef=0.5, sigma_update=True, marginal_update = True, sigma_out=False):
        """
    ...
    def _fit_covariance(self, X_batch, max_workers=4, num_ord_updates=2,
        ...
        if max_workers==1:
            try:
                ...
                np.savetxt("X_batch.txt",X_batch)
                np.savetxt("z_batch.txt",Z)
                np.savetxt("prev_sigma.txt",prev_sigma)
        else:
