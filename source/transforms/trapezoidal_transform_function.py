...
class TrapezoidalTransformFunction():
    ...
    def partial_fit(self, X_batch,cont_indices,ord_indices):
        ...
        if self.window_width<len(X_batch[-1]):
            ...
            temp = np.array([[np.nan for x in range(add_column)] for y in range(self.window_size)]).astype(np.float)
            ...
            temp_pos=np.zeros(add_column).astype(int)
