...
class OnlineTransformFunction():
    ...
    '''
    思路：
    输入的数据为对应的masked batch数据
    构建一个window size，里面所填充的方式分为两种：
    1、序数值
    2、非序数值
    对于序数值
        a.为nan值，采用0-1之间进行随机填充
        b.若非nan值，采用平均值与标准差的范围进行随机填充
    对于非序数值采用
        a.为nan值，直接填充为0
        b.若非nan值，采用最大值与最小值进行随机填充
    
    完成上述填充window之后
    对window（对应batch大小的）进行更新，需要将X_masked对应的batch的非nan数值，更新到window里面
    '''
    ...
    def partial_evaluate_ord_latent(self, X_batch):
        ...
        Z_ord_lower = np.empty(X_ord.shape) # #  返回一个一维或者多维数组，数组的元素不为空，为随机产生的数据
        Z_ord_lower[:] = np.nan
